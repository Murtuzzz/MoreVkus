...
class DrinksController: UIViewController, UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    private func setupCollectionView() {
        ...
        collectionView.register(DrinksCollectionCell.self, forCellWithReuseIdentifier: DrinksCollectionCell.id)
    ...
    private func fetchProducts(from urlString: String, urls: [String], index: Int) {
        ...
        NetworkService.shared.fetchProducts(from: urlString) { [weak self] result in
            ...
            case .success(let products):
                ...
                let filteredProducts = products.filter { $0.category == 6 }
           ...
           let action = userInfo["action"] as? String {
            ...
            if let cardIndex = cardsData.firstIndex(where: { $0.prodId == productId }),
               let displayIndex = displayDataSource.firstIndex(where: { $0.prodId == productId }) {
                ...
                if let cell = collectionView.cellForItem(at: IndexPath(item: displayIndex, section: 0)) as? DrinksCollectionCell {
        ...
        if let userInfo = notification.userInfo,
           let deletedProductId = userInfo["deletedProductId"] as? Int {
            ...
            if let index = cardsData.firstIndex(where: { $0.prodId == deletedProductId }) {
                ...
                if let cell = collectionView.cellForItem(at: IndexPath(item: index, section: 0)) as? DrinksCollectionCell {
           ...
           let productId = userInfo["productId"] as? Int {
            ...
            for i in 0..<displayDataSource.count {
                if displayDataSource[i].prodId == productId {
                    ...
                    if let cell = collectionView.cellForItem(at: IndexPath(item: i, section: 0)) as? DrinksCollectionCell {
...
extension DrinksController: CellDelegate, BasketCellDelegate {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        guard let cell = collectionView.dequeueReusableCell(withReuseIdentifier: DrinksCollectionCell.id, for: indexPath) as? DrinksCollectionCell else {
    ...
    func didTapBasketButton(inCell cell: UICollectionViewCell) {
       ...
       if currentQuantity + 1 >= maxAvailableQuantity {
           if let productCell = cell as? DrinksCollectionCell {
